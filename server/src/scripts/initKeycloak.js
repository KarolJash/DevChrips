...
const clientRoleNames = ["admin", "developer"];

// The realm roles we want for the realm
const realmRoleNames = ["admin", "developer"];
...
async function initKeycloak() {
  ...
  for (let client of ourClients) {
    for (let clientRole of clientRoleNames) {
      console.log(
        `creating client role ${clientRole} for client ${client.clientId}`
      );
      const role = await kc.clients
        .createRole({
          id: client.id,
          name: clientRole,
          clientRole: false,
  ...
  for (let user of users) {
    console.log(
      `creating user ${user.username} with password ${user.password}`
    );
    ...
    for (userRoleName of user.roles) {
      const publicRoleMapping = publicClientRoles.find((role) => {
        return role.name === userRoleName;
      });
      const bearerRoleMapping = bearerClientRoles.find((role) => {
        return role.name === userRoleName;
      });
      const realmRoleMapping = realmRoles.find((role) => {
        return role.name === userRoleName;
      });

      console.log(
        `assigning client and realm roles called "${userRoleName}" to user ${user.username}`
      );
      if (publicRoleMapping) {
        await kc.users.addClientRoleMappings({
          id: u.id,
          clientUniqueId: publicRoleMapping.containerId,
          roles: [publicRoleMapping],
        });
      }
