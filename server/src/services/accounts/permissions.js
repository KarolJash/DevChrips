...
const canPromoteAccount = rule()((parnet, args, { user }, info) => {
  const userPermissions = getPermissions(user);
...
const isEditingOwnAccount = rule()((parent, {where: {id}}. {user}, info) => {
    return user.sub === id;
})
...
const permissions = shield(
  {
    Query: {
        account: or(and(canReadOwnAccount, isReadingOwnAccount), canReadOwnAccount),
