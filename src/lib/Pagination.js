...
class Pagination {
  ...
  async getEdges(queryArgs) {
    ...
    // Upercase will come first, and then lower case. Even if a and Z
  ...
  // Reverse the sort direction when queries need to look in the opposite direction of the set sort order (e.g. next/previous page checks)
  ...
  _getOperator(sort, options = {}) {
    // converts sort, which is an object, to an array
  ...
  // Check if a next page results is available
  async _getHasNextPage(endCursor, filter, sort) {
    ...
    if (isSearch) {
      ...
      nextPage = await this.Model.findOne(queryDoc).select("_id").exec();
    }
